...
Each account has its own wardrobe: garments, outfits, days and photos are scoped by owner in every query, and an id belonging to another account resolves to nothing (`tests/unit/wardrobe-isolation.test.ts`). Reports of one account reaching another's data are in scope.
...
- **Recovery without e-mail**: a person who forgot the password uses a recovery code (100 bits, shown once when the password is chosen, stored as an Argon2id hash) on `/forgot-password`. It is a second credential as strong as the password, it is replaced every time the password changes, and the screen answers the same message for an unknown user, an account without a code and a wrong code. `create-user --reset` clears it.
