...
- `Outfit.paletteId` is a **foreign key into the Sanzo Wada JSON**, not into a DB table. There is no `Palette` model; palettes live in `src/lib/colors/sanzo-wada.json`. It is **nullable**: a combination assembled in the "què em poso?" builder may share no palette at all, and forcing one would be a lie in the data. Read it through `paletteOf` (`src/lib/outfits/worn.ts`), never `paletteMap.get(outfit.paletteId)` directly.
- `Outfit.improvised` marks a combination assembled on the spot in the builder rather than deliberately saved. It is an **explicit column, not `name == null`** — that shortcut looks free (no code path creates a nameless outfit) but real databases already hold nameless *saved* outfits, and reading the missing name as "improvised" hid every one of them from `/armari`. Test it through `isImprovised`. `/armari` hides improvised outfits unless favourited; `/calendari` and stats keep showing them.
