...
- `fit`, `subtype` and `length` are free strings in the data model, so they go through `optionLabel()` in `src/lib/prendas/labels.ts`, which falls back to the raw value for a key this build no longer has. Category, texture, pattern and season are real unions and index `labels` directly. The vocabulary lives in `src/lib/prendas/types.ts`; adding a value there means its label in all three `messages/*.json` (`tests/unit/garment-taxonomy.test.ts` fails otherwise, since `optionLabel` would quietly print the raw key). New subtype keys are English and go at the end of their list, because the older ones are Catalan or Spanish, the key is what an export carries and the list order is the rail order. Fabric, pattern, fit and (except for trousers) length are optional: they are descriptions, and a required field makes people pick a wrong value. Removing a value means checking production rows first.
...
- **Throttling belongs in `authorize()`, not in the login action.** `/api/auth/callback/credentials` is a public endpoint anyone can POST to directly, so a lockout enforced only in the Server Action would be one an attacker walks straight past. The action looks at the same gate (`gateLogin(..., { spend: false })`) too, but only to say "try again in 40 seconds" or "busy" instead of a third wrong-password message; only `authorize()` spends from the global attempt budget.
...
- **The recovery code is the only way back in without the operator.** `User.recoveryHash` holds an Argon2id hash of a 100-bit code (`src/lib/auth/recovery.ts`) that is shown once, when the owner chooses a password (`changePassword` and `recoverAccount` return it, and the forms render `RecoveryCodeNotice` instead of redirecting). The password and its code change together in one `replaceCredentials` write, so a code never outlives its password. `/forgot-password` is public, spends from the same global budget as the login, and answers the same `invalid` for an unknown user, no code on file and a wrong code. Recovering issues a new code, lifts the lockout and, by changing the hash, ends every session and trusted browser. `create-user --reset` clears the code (a temporary password has nothing to recover). Do not store the plain code, and do not add e-mail: the point is that nothing else has to exist.
- **`scripts/create-user.mjs` writes SQL by hand and repeats the Argon2 parameters and the username rule**, because the production image has no Prisma client. `tests/unit/create-user-script.test.ts` runs it against a database built from the migrations and verifies its hash with `password.ts`; a column or cost parameter that changes in one place fails there. It also lists accounts (`--list`) and deletes one with its wardrobe and photos (`--delete --yes`).
...
- **`cropped` is a flag of its own, not a fit** (`Garment.cropped`, only for `OUTERWEAR`, `SWEATER` and `SHIRT`: `canBeCropped`), so a piece can be oversized and cropped. It used to be `fit = CROPPED`; the migration moved those rows, and `upgradeLegacyCropped` (`src/lib/prendas/import.ts`) reads the same thing out of an old export.
