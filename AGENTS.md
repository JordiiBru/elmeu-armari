...
- Serve path: `/api/uploads/[filename]` — the filename regex is a security-relevant allowlist (`[a-z0-9]+(?:-thumb|-cutout)?\.webp`: an id plus at most one companion suffix, no `-`, `.` or `/` in the id part). Do not loosen it without a security review.
...
- **A garment photo may have a cut-out companion** (`<id>-cutout.webp`: the piece on a transparent ground, trimmed to its edges, WebP with alpha). The original is never replaced, and a garment without one falls back to its photograph, which is a supported state, not an error. It is made on upload by `saveGarmentCutout` (`src/lib/uploads.ts`), which sends the 800 px photo to the background removal sidecar at `BG_REMOVAL_URL` (`sidecars/rembg`, U2-Net, deployed in the homelab as `elmeu-armari-rembg`) and never throws: no URL, a slow or failing sidecar, or a mask that keeps under 3 % of the frame all mean no cut-out. A photo that already has a transparent ground is its own cut-out. `deleteUploadImage` removes it, `/api/export/zip` walks it, and `npm run backfill-cutouts` (plain node, reads the garment photos from the database, idempotent, `--all` to redo) fills in the ones that predate it. To know which cut-outs exist use `listCutouts()`, not a stat per garment. Only models with a licence compatible with this MIT repo belong in the sidecar (U2-Net is Apache-2.0); `bria-rmbg`, RMBG and `@imgly/background-removal` (AGPL-3.0) are out.
