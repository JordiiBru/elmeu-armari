...
export function createMigratedDatabase(
  prefix: string,
  { stopBefore }: { stopBefore?: string } = {},
): { dir: string; file: string; migrationsDir: string } {
  ...
  for (const entry of readdirSync(migrations, { withFileTypes: true }).sort((a, b) =>
    ...
    if (stopBefore && entry.name.endsWith(stopBefore)) break;
    db.exec(readFileSync(path.join(migrations, entry.name, "migration.sql"), "utf8"));
  }
  db.close();
  return { dir, file, migrationsDir: migrations };
}
