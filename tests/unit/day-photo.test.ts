...
describe("day photo cleanup", () => {
  it("deletes the photo of a day that is emptied", async () => {
    ...
    await unassignDay("u1", new Date("2026-08-20T09:30:00Z"));

    // Truncated to midnight: the day key, never the click's instant.
    expect(findWornEventImages).toHaveBeenCalledWith("u1", {
      date: new Date("2026-08-20T00:00:00Z"),
    });
    expect(clearWornDay).toHaveBeenCalledWith("u1", new Date("2026-08-20T00:00:00Z"));
  ...
  it("touches no file when the day has no photo", async () => {
    ...
    await unassignDay("u1", new Date("2026-08-20T00:00:00Z"));
  ...
  it("deletes the photos of every day an outfit is deleted from", async () => {
    ...
    await deleteOutfit("u1", "o1");

    expect(findWornEventImages).toHaveBeenCalledWith("u1", { outfitId: "o1" });
    expect(deleteOutfitRow).toHaveBeenCalledWith("u1", "o1");
    expect(deleteUploadImage).toHaveBeenCalledWith("we1");
